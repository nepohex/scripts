...
$includes_dir = __DIR__ . "/includes/"; //Директория со всеми инклудами

//include $includes_dir . 'spin_tpls.php'; // Будет использован текстовый файлик обычный из папки инклудов spin.txt, править вручную.
include "../includes/functions.php";

//Обязательные к изменению функции от сайта к сайту
$site_name = 'mfa_humanbody14.com'; // Без слешей, только домен
$db_name = $site_name;
$keyword = 'human'; //просто ключ, использщуется только для генерации базы и пользователей к ней
$blogname = ucwords("Human Anatomy Charts");
$blogdescription = $blogname . ". Free Encyclopedia for Students - Learn with Ease.";
...
$images_per_site = 20000; // Сколько картинок брать на 1 сайт (без учета их размера, еще может сильно сократиться, обычно на 20% в итоге выходит)
...
$cats = 50; // Сколько категорий автоматом создать
...
$min_img_size = 20000; // размер в байтах картинки минимальный
$max_img_size = 20000000;
$seasonal_add = FALSE; // Будем к Title дописывать год, ниже % скольки тайтлам
...
$default_cat_name = 'Encyclopedia'; //Название и URL стандартной (1) категории WP, сюда попадет все что не попало в другие категории.
$default_cat_slug = 'encyclopedia'; // URL категории default (1)
...
$wp_image_upload_date_prefix = '2018/04/';
...
$db_instance = $includes_dir . 'db_instance_052018.sql'; // Пустая база данных с таблицами Wordpress, которая будет создаваться каждый раз для нового сайта. Лежать будет пока в папке со скриптом.
$db_usr = 'root';
$db_name = $site_name;
...
$bad_symbols = array('-', '_', '39s', '$', '%', '^', '&', '(', ')', '=', '+', '=', '`', '~', '\'', ']', '[', '{', '}', ',', '.', '"', '  '); //Заменим эти символы в имени файла на пробелы
