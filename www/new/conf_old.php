...
#done разнести наконец все зависимые файлы в папку инклудов
#done Нашел баг, 130к картинок в базе image_index не имеют записи Domain, надо восстановить по файлам импорта и прописать домены
#done Поправить базу, убрать my_spintax таблицу.
#todo Чтобы не было полных дублей Title в пределах сайта
#done Отказался от идеи imagemagick 15ым шагом сделать выборочно true/false -> Отказался от идеи т.к. картинки в размере увеличиваются или остаются неизменными, а XNVIEW дает в 2-3 раза меньший размер.
#done Загрузку Spintax надо в базу image_index и брать оттуда, не передавать в instance базы.
#todo сделать возможность работать сразу по нескольким ключам и словосочетаниям в пределах одного сайта
#done чтобы не захламлять папку рабочую вынести файлы с выгрузками CSV в отдельную папку, если напрямую путь поменять то там за собой тянет еще кучу всего
...
#done функцию подбора неиспользованных по слову картинок, добора оставшихся. Например в Highlight 20000 фоток, 10000 взял 1 сайт, надо взять только уникальные другие 10000. Делать в файле 2 при выборе из общего массива данных картинки, помечать их как "взятые".
