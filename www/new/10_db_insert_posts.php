...
foreach ($wp_postmetas as $item) {
    ...
    $queries[] = "INSERT INTO `wp_term_relationships` (`object_id`, `term_taxonomy_id`, `term_order`) VALUES (" . $post_guid . ", 1, 0);";

    //Update post parent для Attachment.
    $queries[] = "UPDATE `wp_posts` SET `post_parent` = $post_guid WHERE `id` = " . $item['post_id'] . ";";
    dbquery($queries);
    unset($queries);
    $post_guid++;
}
