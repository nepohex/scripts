...
<div class="container">
<span style="font-family: Georgia;">Статистика посещаемости</span>
...
foreach ($sites as $site) {
	...
	if($max>=0 && $pre>0) {
		echo "<td>".number_format($csv[$domain][$max]['visitors'])." ".$this->diff($csv[$domain][$max]['visitors'],$csv[$domain][$pre]['visitors'])."
		<td>".number_format($csv[$domain][$max]['views'])." ".$this->diff($csv[$domain][$max]['views'],$csv[$domain][$pre]['views']);
	} else {
		echo "<td>0<td>0";
	}

	
	echo "<td><a style=\"cursor: pointer; color: red;\" title=\"Удалить сайт\" onclick=\"del('$domain');\">&times;</a>";
